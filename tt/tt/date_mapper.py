...
def _translate_format(m: re.Match[str]) -> str:
    ...
    fmt_arg = m.group(2).strip().strip("'\"")
    # Map date-fns format tokens to strftime
    py_fmt = (
        fmt_arg
        .replace("yyyy", "%Y")
        .replace("MM", "%m")
...
def _translate_each_day_of_interval(m: re.Match[str]) -> str:
    # eachDayOfInterval({start, end}) → each_day_of_interval(start, end)
...
_TRANSLATIONS: list[tuple[re.Pattern[str], object]] = [
    (re.compile(r"\bformat\s*\(\s*([^,]+),\s*(['\"][^'\"]+['\"])\s*\)"), _translate_format),
    ...
    (re.compile(r"\beachDayOfInterval\s*\(\s*(\{[^}]*\})\s*\)"), _translate_each_day_of_interval),
