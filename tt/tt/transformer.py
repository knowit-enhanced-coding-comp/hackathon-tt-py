...
def translate_expression(node: Node, source: bytes, context: dict) -> str:
    ...
    if t == "string":
        ...
        # Normalise to double-quoted Python string
    ...
    if t == "await_expression":
        ...
        return translate_expression(inner_node, source, context)

    # New expression
    if t == "new_expression":
        return _translate_new(node, source, context)

    # As expression (TypeScript cast)
    if t == "as_expression":
    ...
    if t == "spread_element":
        inner_node = node.children[-1]
        return f"*{translate_expression(inner_node, source, context)}"
...
def translate_statement(node: Node, source: bytes, context: dict, indent: int = 0) -> str:
    ...
    if t == "expression_statement":
        ...
        expr = translate_expression(expr_node, source, context)
        return f"{pad}{expr}"
    ...
    for child in node.children:
        if child.type == "variable_declarator":
            ...
            if name_node.type == "array_pattern":
                ...
                lines.append(f"{pad}{', '.join(names)} = {arr_val}")
                continue
...
def _translate_if(node: Node, source: bytes, context: dict, indent: int) -> str:
    ...
    # Find condition and consequence
    cond_node = None
    cons_node = None
    alt_node = None
    ...
    if cons_node:
        body = translate_statement(cons_node, source, context, indent + 1)
        lines.append(body)
    else:
        lines.append(f"{pad}    pass")

    if alt_node:
        # else clause children: else <statement>
        alt_body = [c for c in alt_node.children if c.type not in ("else",)]
        if alt_body:
            alt_stmt = alt_body[0]
