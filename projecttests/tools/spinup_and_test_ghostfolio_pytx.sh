#!/usr/bin/env bash
# Start the translated Python Ghostfolio API skeleton, run the integration
...
#
# Options (env vars):
#   PYTX_PORT   Host port to bind (default: 3334)
...
# ---------------------------------------------------------------------------
echo "Starting Python Ghostfolio skeleton (port $PORT)..."
"$UV" run --project "$PYTX_DIR" uvicorn app.main:app \
  --host 127.0.0.1 \
  --port "$PORT" \
  --log-level warning \
  &
...
# ---------------------------------------------------------------------------
if [ "${KEEP_UP:-0}" != "1" ]; then
  echo "Stopping Python skeleton server..."
  stop_server
fi
